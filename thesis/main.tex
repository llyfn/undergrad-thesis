...
% --- MAIN BODY ---
    ...
    Recent advancements in Natural Language Processing (NLP) have seen the widespread adoption of Pre-trained Language Models such as BERT~\cite{devlin2019bert} and RoBERTa~\cite{liu2019roberta}.
    These models have achieved state-of-the-art performance on sarcasm detection benchmarks, most notably the FigLang 2020 Shared Task~\cite{ghosh2020report}, where Transformer-based systems dominated the leaderboard by leveraging conversational history.
    ...
    By explicitly optimizing the encoder to minimize intra-class variance and maximize inter-class variance of the feature representations, we generate an embedding space that is structurally more conducive to linear classification.
    ...
    Wallace et al. (2014)~\cite{wallace2014humans} demonstrated that humans often require context to identify ironic intent, implying that computational models share this requirement.
    ...
    Unlike standard classification tasks that utilize the \texttt{[CLS]} token, this stage utilizes a mean-pooling operation over the token embeddings.
    This is because the \texttt{[CLS]} token is often biased towards next-sentence prediction tasks from pre-training, whereas mean-pooling aggregates information from the entire sequence, producing a more robust holistic representation for clustering objectives~\cite{li2021simclad}.
    ...
    \end{center}
    This format is effective since the Transformer architecture utilizes a self-attention mechanism.
    By placing both context and response in the same sequence, self-attention allows every token in the response to directly attend to every token in the context layers.
    This enables the model to effectively capture the context incongruity which is the hallmark of sarcasm.
    ...
    The statistics of context length for both datasets are visualized in Figure~\ref{fig:context_len}.
    \begin{itemize}
        \item \textbf{FigLang-Reddit:} Comprises 4,400 training samples and 1,800 test samples.
        It is derived from the self-annotated Reddit corpus by Khodak et al. (2018)~\cite{khodak2018large}, where users mark sarcasm using the \texttt{/s} tag.
        As shown in Figure~\ref{fig:context_len}, the dataset is characterized by short context lengths, with over 60\% of samples containing only 2 prior turns.
        \item \textbf{FigLang-Twitter:} Comprises 5,000 training samples and 1,800 test samples.
        Sarcastic tweets are collected using hashtags such as \#sarcasm and \#sarcastic.
        Unlike Reddit, the non-sarcastic examples are not random tweets but are sentiment-bearing tweets (containing hashtags like \#happy, \#sad), making the discrimination task considerably harder~\cite{ghosh2020report}.
        Figure~\ref{fig:context_len} illustrates that Twitter threads are longer, with a notable distribution of samples having more than 5 context turns.
    \end{itemize}
    ...
    \begin{figure}[t]
        \centering
        \input{dataset}
        \caption{Distribution of context lengths in the training datasets.}
        \label{fig:context_len}
    \end{figure}
    ...
    \begin{table}[H]
        ...
        \begin{tabular}{l c c c c c}
            \toprule
            \textbf{Dataset} & \textbf{Split} & \textbf{Model} & \textbf{Precision} & \textbf{Recall} & \textbf{Macro F1} \\
            \midrule
            \multirow{2}{*}{Reddit} & \multirow{2}{*}{Dev} & Baseline & 0.5679 & 0.5568 & 0.5379 \\ & & \textbf{SimSCLSD} & \textbf{0.6974} & \textbf{0.6955} & \textbf{0.6947} \\
            \midrule
            \multirow{2}{*}{Reddit} & \multirow{2}{*}{Test} & Baseline & 0.5244 & 0.5228 & 0.5148 \\ & & \textbf{SimSCLSD} & \textbf{0.6166} & \textbf{0.6139} & \textbf{0.6116} \\
            \midrule
            \multirow{2}{*}{Twitter} & \multirow{2}{*}{Dev} & Baseline & 0.7352 & 0.7300 & 0.7285 \\ & & \textbf{SimSCLSD} & \textbf{0.8486} & \textbf{0.8480} & \textbf{0.8479} \\
            \midrule
            \multirow{2}{*}{Twitter} & \multirow{2}{*}{Test} & Baseline & 0.7047 & 0.6861 & 0.6788 \\ & & \textbf{SimSCLSD} & \textbf{0.7477} & \textbf{0.7461} & \textbf{0.7457} \\
            \bottomrule
        \end{tabular}
    \end{table}
